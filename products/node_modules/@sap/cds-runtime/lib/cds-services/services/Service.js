...
class Service extends Client {
  ...
  _addDefaultBeforeAuthHandler () {
    for (const entity of this._getServiceEntities()) {
      ...
      for (const key of Object.keys(annotations)) {
        ...
        this._initial(key, entity, this._markDefault(handler))
      }
  ...
  async processEvent (context) {
    ...
    try {
      ...
      // OKRA expects a stack
      err.stack = err.stack || 'not available'
  ...
  async _commitRollback (command, context, result) {
    if (
      context._.shared.doNotFinishTransaction === undefined &&
      (!context._.shared.atomicityGroup || !context._.shared.atomicityGroup[context._txId])
    ) {
  ...
  async _executeFinishTransactionHandlers (command, context, dataOperations) {
    ...
    try {
      ...
      // OKRA expects a stack
      commandErr.stack = commandErr.stack || 'not available'
  ...
  _beforeEndTransactionBlock (command, context, dataOperations) {
    ...
    for (const dataOperation of dataOperations) {
      ...
      const newContext = Object.assign(dataOperation, {
        _: context._,
        _txId: context._txId,
        run: (...args) => {
          return global.cds.db.transaction(context).run(...args)
        }
      })
