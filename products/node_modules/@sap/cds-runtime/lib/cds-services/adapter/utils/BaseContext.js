...
class BaseContext extends DelayedEmitter {
  ...
  postPreparation (incomingRequest) {
    ...
    } else if (incomingRequest.getBatchContext) {
      const batchContext = incomingRequest.getBatchContext()
      if (
        batchContext &&
        incomingRequest
          .getBatchContext()
          .getRequest()
          .getIncomingRequest().dynatrace
      ) {
        this.dynatrace = batchContext.getRequest().getIncomingRequest().dynatrace
      }
    }
  }
