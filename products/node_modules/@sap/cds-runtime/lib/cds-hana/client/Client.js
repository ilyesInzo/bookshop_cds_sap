...
class Client extends BaseClient {
  ...
  constructor ({ credentials }) {
    ...
    this._isHanaClient = hanaClient.name === '@sap/hana-client'
    this._credentials = credentials
    this._user = 'ANONYMOUS' // Use anonymous HANA user as default
    this._transCount = 0

    if (this._isHanaClient) {
      this._dbc = hanaClient.createConnection()
  ...
  connect () {
    return new Promise((resolve, reject) => {
      const cb = err => {
        ...
        let finalize
        if (!this._isHanaClient && this._credentials.schema) {
          finalize = this.execute(`SET SCHEMA ${this._credentials.schema}`)
        } else {
      ...
      if (this._isHanaClient) {
        ...
        if (this._credentials.schema) {
          connParams.CURRENTSCHEMA = this._credentials.schema
        }
        this._traced(this._dbc.connect, Object.assign({}, this._credentials, connParams), cb)
      } else {
        this._traced(this._dbc.connect, cb)
      }
    })
  }
  ...
  _execute (cqn, inValues = []) {
    ...
    const { sql, values = [] } = cdsSql.builder.sqlFactory(
      ...
      {
        now: this._transactionTimestamp || { sql: 'NOW ()' },
  ...
  _executeSQL (sql, values, isOne, postMapper, propertyMapper, objStructMapper, useHanaClientStatement) {
    ...
    return new Promise((resolve, reject) => {
      this._traced(this._dbc.exec, sql, (err, result) => {
        if (err) {
          // make sure err is an instance of Error for robustness
          if (!(err instanceof Error)) {
            err = Object.assign(new Error(), err)
          }
  ...
  _preparedExecuteCb (sql, useHanaClientStatement, values, client, cb) {
    client
      ...
      .catch(error => {
        if (error.code === 613) {
          cb(new Error('Record locked by concurrent transaction'))
        }

        // make sure error is an instance of Error for robustness
    ...
    if (this._transCount === 0) {
      return new Promise((resolve, reject) => {
        this._traced(this._dbc.commit, err => {
          // auto-commit is true by default, i.e., reset client to initial state
